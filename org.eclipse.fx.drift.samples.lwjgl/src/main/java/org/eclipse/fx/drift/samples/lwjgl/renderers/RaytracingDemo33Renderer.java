...
public class RaytracingDemo33Renderer implements ISampleRenderer, IWithListeners  {
...
//    }
    ...
    public String toString() {
    	return "lwjgl - raytracing demo33";
    }
    ...
    public Subscription installListeners(DriftFXSurface surface) {
		EventHandler<? super MouseEvent> onMouseMoved = event -> {
			...
			RaytracingDemo33Renderer.this.mouseX = (float)x;
            if ( mouseDown ) {
                RaytracingDemo33Renderer.this.frameNumber = 0;
            }
		};
		EventHandler<? super MouseEvent> onMousePressed = event -> {
            RaytracingDemo33Renderer.this.mouseDownX = RaytracingDemo33Renderer.this.mouseX;
            RaytracingDemo33Renderer.this.mouseDown = true;
		};
		EventHandler<? super MouseEvent> onMouseReleased = event -> {
            RaytracingDemo33Renderer.this.mouseDown = false;
            RaytracingDemo33Renderer.this.rotationAboutY = RaytracingDemo33Renderer.this.currRotationAboutY;
		};
    ...
    public void initialize() throws Exception {
    	// create a GL context
		ctx = org.eclipse.fx.drift.internal.GL.createContext(0, 3, 3);
    ...
    public void render(Vec2i size, RenderTarget target) {
    	...
    	if (width != size.x || height != size.y) {
			RaytracingDemo33Renderer.this.width = size.x;
			RaytracingDemo33Renderer.this.height = size.y;
			RaytracingDemo33Renderer.this.resetFramebuffer = true;
			RaytracingDemo33Renderer.this.frameNumber = 0;
		}
