...
public class TwoSurfacesTwoRenderThreads implements IDriftFXSampleProvider {
	
	@Override
	public String getTitle() {
		return "lwjgl - 2 surfaces 2 threads";
	}
	...
	public Node createInstance() {
		...
		SceneRendererThread thread = new SceneRendererThread(demoRenderer1);
		...
		SceneRendererThread thread2 = new SceneRendererThread(demoRenderer2);
		
		
		HBox control2 = new HBox();
		
		Button start2 = new Button("start");
		start2.setMinWidth(Region.USE_PREF_SIZE);
		Button stop2 = new Button("stop");
		stop2.setMinWidth(Region.USE_PREF_SIZE);
		
		start2.setOnAction(e -> thread2.start());
		stop2.setOnAction(e -> thread2.stop());
		
		control2.setSpacing(5);
		control2.getChildren().addAll(start2, stop2);
		
		grid.add(control2, 1, 2);
